"""
analyze_top_8.py — Run TrojanLogic4H on top 8 cryptos
"""
import sys
import os
...
# Top 8 symbols (excluding stablecoins)
TOP_8 = [
    "BTCUSDT",
    "ETHUSDT", 
    "SOLUSDT",
...
def analyze_all():
    """Analyze top 8 and generate report."""
    print("=== TROJANLOGIC4H: TOP 8 CRYPTO ANALYSIS ===\n")
    ...
    # Save results
    output_file = f"top_8_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    if opportunities:
        ...
        for opp in sorted(opportunities, key=lambda x: x['confidence'], reverse=True):
            ...
            print(f"    Setup: {opp['setup_type']}")
            if opp.get('entry_zone'):
                print(f"    Entry: ${opp['entry_zone'][0]:,.2f} - ${opp['entry_zone'][1]:,.2f}")
            if opp.get('stop_loss'):
