# Polymarket Telegram Alert - CLEAN VERSION
# Sends formatted alerts with proper value extraction
...
if ($Content -match "NO HOT BETS") {
    ...
    if ($Lines[1] -match "Found (\d+) opportunities") {
        $Count = $Matches[1]
        $Message += "Found $Count opportunities`n`n"
    }
    
    # Parse bets - extract info properly
    $BetCount = 0
    for ($i = 0; $i -lt $Lines.Count; $i++) {
        ...
        if ($Line -match '^\[([A-Z]+)\]\s+(.+)$') {
            ...
            # Add to message with simple bullet
