# Polymarket Telegram Alert - ENHANCED VERSION
# Shows bet recommendation and file location
...
# Get the folder path for full file link
$FolderPath = Split-Path $AlertFile -Parent
$FolderName = Split-Path $FolderPath -Leaf
$FileName = Split-Path $AlertFile -Leaf
...
if ($Content -match "NO HOT BETS") {
    ...
    for ($i = 0; $i -lt $Lines.Count; $i++) {
        ...
        if ($Line -match '^\[([A-Z]+)\]\s+(.+)$') {
            ...
            # Determine recommendation based on market name
            $Rec = ""
            if ($Name -match "vs\.?") {
                # Sports match - extract teams
                $Rec = "Check market for teams"
            } elseif ($Name -match "\?") {
                # Yes/No question
                $Rec = "Check Yes/No odds"
            }
    ...
    # Add file location
    $Message += "---`n"
    $Message += "Full list: Documents\\Polymarket-Scans\\$FolderName\\$FileName`n"
    $Message += "All $Count opportunities saved locally"
}
